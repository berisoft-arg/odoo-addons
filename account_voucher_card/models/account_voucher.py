...
class AccountVoucherOperation(models.Model):

    _name = 'account.voucher.operation'
    _description = 'account.voucher.operation'
    _rec_name = 'operation'
        ...
        for rec in self:
            if rec.origin:
                raise ValidationError(_(
                    'You can not delete a voucher operation that has an origin.'
                    '\nYou can delete the origin reference and unlink after.'))
        return super(AccountVoucherOperation, self).unlink()
    ...
    def _compute_origin_name(self):
        ...
        for rec in self:
            try:
                ...
                _logger.exception(
                    "Compute origin on vouchers exception: %s" % e)
...
class AccountVoucher(models.Model):
    ...
    type = fields.Selection(
        [('issue_voucher', 'Voucher Emitido'), ('third_voucher', 'Voucher de Terceros')],
        readonly=True,
        index=True,
    )
    partner_id = fields.Many2one(
    ...
    def get_bank_vals(self, action, journal):
        ...
        if action == 'bank_debit':
            ...
            debit_account = self.company_id._get_voucher_account('deferred')
            name = _('Voucher "%s" debit') % (self.name)
            ...
            credit_account = journal.default_account_id
            debit_account = self.company_id._get_voucher_account('rejected')
            name = _('Voucher "%s" rejection') % (self.name)
        elif action == 'bank_deposit' or action == 'bank_sell':
            # al transferir a un banco se usa esta. al volver tiene que volver
            # por la opuesta
            # self.destination_journal_id.default_credit_account_id
            name = _('Voucher "%s" deposit') % (self.name)
            if action == 'bank_deposit':
                  debit_account = journal.default_account_id
                  credit_account = self.company_id._get_voucher_account('holding')
            if action == 'bank_sell' and not self.company_id.negotiated_voucher_account_id:
                  raise ValidationError('No esta definida la cuenta de vouchers negociados a nivel empresa')
            if action == 'bank_sell':
                  debit_account = self.company_id.negotiated_voucher_account_id
                  credit_account = self.company_id._get_voucher_account('holding')
                  if not credit_account:
                        raise ValidationError('Falta la cuenta holding_voucher_account_id')
                  if not debit_account:
                        raise ValidationError('Falta la cuenta negotiated_voucher_account_id')
                  if action == 'bank_deposit':
                        name = _('Voucher "%s" deposit') % (self.name)
                  else:
                        name = _('Voucher "%s" sell') % (self.name)
        else:
            raise ValidationError(_(
                                'Action %s not implemented for vouchers!') % action)
    ...
    def onchange(self, values, field_name, field_onchange):
        ...
        return super(AccountVoucher, self).onchange(
    ...
    def issue_number_interval(self):
        for rec in self:
            # if not range, then we dont voucher it
            if rec.type == 'issue_voucher' and rec.voucherbook_id.range_to:
                if rec.number > rec.voucherbook_id.range_to:
                    raise UserError(_(
                        "Voucher number (%s) can't be greater than %s on "
                        "voucherbook %s (%s)") % (
                        rec.number,
                        rec.voucherbook_id.range_to,
                        rec.voucherbook_id.name,
                        rec.voucherbook_id.id,
                    ))
                elif rec.number == rec.voucherbook_id.range_to:
                    rec.voucherbook_id.state = 'used'
        return False
    ...
    def _voucher_unique(self):
        for rec in self:
            if rec.type == 'issue_voucher':
                same_vouchers = self.search([
                    ('voucherbook_id', '=', rec.voucherbook_id.id),
                    ('type', '=', rec.type),
                    ('number', '=', rec.number),
                ])
                same_vouchers -= self
                if same_vouchers:
                    raise ValidationError(_(
                        'Voucher Number (%s) must be unique per Voucherbook!\n'
                        '* Voucher ids: %s') % (
                        rec.name, same_vouchers.ids))
            elif self.type == 'third_voucher':
                # agregamos condicion de company ya que un voucher de terceros
                ...
                if same_vouchers:
                    raise ValidationError(_(
                        'Voucher Number (%s) must be unique per Owner and Bank!'
                        '\n* Voucher ids: %s') % (
                        rec.name, same_vouchers.ids))
        return True
    ...
    def _del_operation(self, origin):
        """
        We voucher that the operation that is being cancel is the last operation
        done (same as voucher state)
        """
        for rec in self:
            if not rec.operation_ids or rec.operation_ids[0].origin != origin:
                raise ValidationError(_(
                    'You can not cancel this operation because this is not '
                    'the last operation over the voucher.\nVoucher (id): %s (%s)'
                ) % (rec.name, rec.id))
    ...
    def _add_operation(
        ...
        for rec in self:
            rec._voucher_state_change(operation)
            ...
            if rec.operation_ids and rec.operation_ids[0].date > date:
                raise ValidationError(_(
                    'The date of a new voucher operation can not be minor than '
                    'last operation date.\n'
                    '* Voucher Id: %s\n'
                    '* Voucher Number: %s\n'
                    '* Operation: %s\n'
    ...
    def _voucher_state_change(self, operation):
        """
        We only voucher state change from _add_operation because we want to
        leave the user the possibility of making anything from interface.
        Necesitamos este chequeo para evitar, por ejemplo, que un voucher se
        ...
        * value is 'from states'
        ...
        if not from_states:
            raise ValidationError(_(
                'Operation %s not implemented for vouchers!') % operation)
        if old_state not in from_states:
            raise ValidationError(_(
                'You can not "%s" a voucher from state "%s"!\n'
                'Voucher nbr (id): %s (%s)') % (
    ...
    def unlink(self):
        for rec in self:
            if rec.state not in ('draft', 'cancel'):
                raise ValidationError(_(
                    'The Voucher must be in draft state for unlink !'))
        return super(AccountVoucher, self).unlink()

# vouchers operations from vouchers
    ...
    def bank_debit(self):
        ...
        if self.state in ['handed']:
            ...
            #    default_name=_('Voucher "%s" debit') % (self.name),
            #    force_account_id=self.company_id._get_voucher_account(
            #        'deferred').id,
            #).create(payment_values)
            #self.post_payment_voucher(payment)
            #payment.post()
    ...
    @api.model
    def post_payment_voucher(self, payment):
    ...
    def handed_reconcile(self, move):
        """
        Funcion que por ahora solo intenta conciliar vouchers propios entregados
        ...
        if debit_account.reconcile:
            ...
            if len(move_lines) != 2:
                raise ValidationError(_(
                    ...
                    'reconcile with voucher debit.\n'
                    '*Journal items: %s') % move_lines.ids)
            move_lines.reconcile()

    @api.model
    def get_third_voucher_account(self):
        """
        For third vouchers, if we use a journal only for third vouchers, we use
        accounts on journal, if not we use company account
        # TODO la idea es depreciar esto y que si se usa vouchers de terceros
        ...
        """
        # self.ensure_one()
        # desde los pagos, pueden venir mas de un voucher pero para que
        # funcione bien, todos los vouchers deberian usar la misma cuenta,
        ...
        for rec in self:
            ...
            debit_account = credit_account = rec.company_id._get_voucher_account('holding')
            ...
            if credit_account and credit_account == debit_account and len(
                ...
                account |= rec.company_id._get_voucher_account('holding')
        if len(account) != 1:
            raise ValidationError(_('Error not specified'))
        return account

    @api.model
    def _get_vouchers_to_date_on_state(self, state, date, force_domain=None):
        """
        Devuelve el listado de vouchers que a la fecha definida se encontraban
        ...
        cuales se puede acceder al voucher, devolvemos las operaciones porque
        ...
        for operation in operations:
            # buscamos si hay alguna otra operacion posterior para el voucher
            newer_op = operation.search([
                ('date', '<=', date),
                ('id', '>', operation.id),
                ('voucher_id', '=', operation.voucher_id.id),
            ])
            # si hay una operacion posterior borramos la op del voucher porque
    ...
    def _get_operation(self, operation, partner_required=False):
        ...
        op = self.operation_ids.search([
            ('voucher_id', '=', self.id), ('operation', '=', operation)],
            limit=1)
        if partner_required:
    ...
    def claim(self):
        self.ensure_one()
        if self.state in ['rejected'] and self.type == 'third_voucher':
            # anulamos la operación en la que lo recibimos
            return self.action_create_debit_note(
                'reclaimed', 'customer', self.first_partner_id,
                self.company_id._get_voucher_account('rejected'))

    def customer_return(self):
        self.ensure_one()
        if self.state in ['holding'] and self.type == 'third_voucher':
            return self.action_create_debit_note(
                'returned', 'customer', self.first_partner_id,
                self.get_third_voucher_account())

    @api.model
    def get_payment_values(self, journal):
        """ return dictionary with the values to create the reject voucher
        payment record.
    ...
    def reject(self):
        ...
        if self.state in ['deposited', 'selled']:
            ...
            payment = self.env['account.payment'].with_context(
                default_name=_('Voucher "%s" rejection') % (self.name),
                #force_account_id=self.company_id._get_voucher_account(
                #    'rejected').id,
            ).create(payment_vals)
            # self.post_payment_voucher(payment)
            ...
            return self.action_create_debit_note(
                'rejected', 'supplier', operation.partner_id,
                self.company_id._get_voucher_account('deferred'))
    ...
    def action_create_debit_note(
        ...
        if operation in ['rejected', 'reclaimed']:
            name = 'Rechazo voucher "%s"' % (self.name)
        # si pedimos la de holding es una devolucion
        elif operation == 'returned':
            name = 'Devolución voucher "%s"' % (self.name)
        else:
            raise ValidationError(_(
        ...
        inv_vals = {
            ...
            'rejected_voucher_id': self.id,
            #'ref': name,
            'invoice_date': action_date,
            'ref': _('Voucher nbr (id): %s (%s)') % (self.name, self.id),
            'journal_id': journal.id,
